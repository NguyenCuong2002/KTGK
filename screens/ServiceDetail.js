...
const ServiceDetail = ({ route, navigation }) => {
  ...
  const handleOrderConfirm = async () => {
    try {
      ...
      Alert.alert("Success", `Đặt lịch ${serviceName} thành công`);
      // setPhoneNumber('');
      // setCustomerName('');
  ...
  return (
    <View>
      <View style={{ padding: 10 }}>
        <Text style={{ fontSize: 17, fontWeight: 'bold', color: 'black' }}>Tên dịch vụ: {serviceName}</Text>
        <Text style={{ fontSize: 12, fontWeight: 'bold', color: 'black' }}>Giá: {price} VNĐ</Text>
      </View>
  ...
  />
      ...
      >
        <Text style={{ color: '#fff', fontSize: 18, fontWeight: 'bold' }}>Đặt lịch</Text>
